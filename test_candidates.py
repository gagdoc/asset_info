import time
from backend.services.assets_service import load_from_db
import pandas as pd
...
def get_search_candidates():
    ...
    for c in candidates:
        ...
        if c["BU"] and merged[key]["BU"] in ["", "-"]:
            merged[key]["BU"] = c["BU"]
        if c["ROLE"] and merged[key]["ROLE"] in ["", "-"]:
    ...
    for m in merged.values():
        ...
        for f in ["Lease_List", "Ipad_List", "Printer", "Monitor", "TeamsNum"]:
            res[f] = ", ".join(m[f]) if m[f] else "-"
        result.append(res)
        
    return result

c = get_search_candidates()
print("Total candidates:", len(c))
print("Stock:", [x for x in c if 'STOCK' in x['이름'].upper()])
